...
- 現在実装済みの範囲: 4画面とナビゲーション、アプリ起動からの経過時間表示、自動起動設定、テスト通知、起動1分後の仮通知、システムトレイ、前面ウィンドウ情報の単発取得、デフォルト時給とWindowsアプリ別時給の登録・保存・解決、Chrome拡張機能とNative Messaging HostによるWebアプリ検出、Tauri Command、型・値検証付きフロントサービス。
- 現在未実装の範囲: 前面ウィンドウの継続監視、Windowsアプリ別利用時間の計測、SQLiteへの保存、分類ルールの作成・適用、活動履歴表示、実際の利用条件に基づく通知、集計、グラフ、外部通信。
...
- Rust 側が担当する処理: Tauriウィンドウ制御、Windows APIによる前面ウィンドウ情報の単発取得、Tauri Command、Store Plugin初期化、システムトレイ、起動5秒後の仮通知。
- Tauri Command の呼び出し構造: `getActiveWindowInfo()` が `@tauri-apps/api` の `invoke("get_active_window_info")` を呼び、戻り値をZodで検証する。タイマー画面からはまだ呼び出していない。
...
- 現在のタイマー表示の流れ: `App.tsx` が起動時刻をZustandへ保存し、1秒ごとに実経過時間を同期してタイマー画面へ表示する。この値はアプリ別の利用時間ではない。
...
- Webアプリ計測の流れ: Chrome拡張機能が成功ACKを受けたURLだけを送信済みとして保存し、Native Messaging HostがTauriへ転送する。フロントはイベントをZodで検証し、対応WebアプリのZustandセッションを開始・終了する。
...
- 現時点でどこまで実装されているか: 4画面の切り替え、起動経過タイマー、自動起動設定、時給設定、開発用仮通知を実装済み。カレンダーとグラフはプレースホルダー。
...
- 現時点でどこまで実装されているか: タイマーは起動経過時間、Chrome拡張機能の接続状態、現在のWebアプリ、利用時間を表示する。設定は自動起動、デフォルト時給、前面Windowsアプリの登録、アプリ別上書き時給を操作できる。カレンダーとグラフはページ名と説明のみ。
- どのファイルから呼ばれる予定か: `src/App.tsx` から呼ばれる。
...
- 現在動作する機能: 4画面の切り替え、アプリ起動からの経過時間表示、自動起動の切り替え、テスト通知、起動1分後の仮通知、トレイ常駐と再表示・終了、前面ウィンドウ情報の単発取得、Chrome上の対応Webアプリ検出とセッション時間表示、デフォルト時給とWindowsアプリ別時給の登録・保存・変更・解除・再起動復元、分類済みレコードの金額換算と集計を行うTypeScript純粋関数。
...
- 未実装の機能: 前面ウィンドウの継続監視、Windowsアプリ別時間計測、活動履歴のSQLite保存、分類、履歴表示、金額結果のUI表示・保存への接続、クラウド通信。
