...
- React・TypeScript 側が担当する処理: 画面表示、ナビゲーション、設定入力、サービス層を通したCommand呼び出し、UI状態管理。
- Rust 側が担当する処理: Tauriウィンドウ制御、Windows APIによる前面ウィンドウ情報の単発取得、保存処理の仲介、通知、常駐まわり。
...
- 現時点でどこまで実装されているか: `activityService.ts` に前面ウィンドウ情報を1回取得し、nullableなZodスキーマで検証する `getActiveWindowInfo()` がある。
...
- 現時点でどこまで実装されているか: 型に対応するスキーマに加え、`activeWindowInfoSchema` がTauri Commandの戻り値検証に利用されている。
...
- 現時点でどこまで実装されているか: 前面ウィンドウ取得のplatform処理とTauri Commandが実装・登録されている。
...
- 現在動作する最低限の機能: 4画面の切り替え、ウィンドウ起動、前面ウィンドウ情報の単発取得とTauri Command・型検証付きフロントサービス。
...
- 未実装の機能: 前面ウィンドウの継続監視・保存・分類・画面表示、集計、ブラウザ拡張機能、クラウド通信。
- 将来実装する機能: 活動履歴取得、ルール適用、SQLite 保存、通知、常駐、月次集計。
- Windows 限定の処理: 前面ウィンドウの単発取得。アイドル検知などは将来の対象。
- 今回導入していないライブラリやプラグイン: Tauri SQL Plugin、Tauri Store Plugin、Tauri Notification Plugin、Tauri Autostart Plugin、TrayIcon。

## 10. セキュリティとプライバシー

- 単発取得できる情報: 前面ウィンドウのプロセス名、ウィンドウタイトル、PID。利用時間、分類結果、設定値の収集・保存は未実装。
