...
export function AppUsageLimitSettingsSection() {
  ...
  return <section className="app-usage-limit-settings">
    <h3>アプリ別利用制限</h3>
    {error ? <p role="alert">{error}</p> : null}
    <form className="app-usage-limit-settings__form" onSubmit={(event) => { event.preventDefault(); void submit(); }}>
      ...
      <label>一日の上限（分） <input type="number" min="1" value={draft.dailyLimitMinutes} onChange={(event) => setDraft({ ...draft, dailyLimitMinutes: event.target.value })} /></label>
      <br className="app-usage-limit-settings__field-break" />
      ...
      <div className="hourly-rate-settings__app-actions">
        <button className="hourly-rate-settings__save-button" type="submit">{editingAppId === null ? "追加" : "更新"}</button>
        {editingAppId !== null ? <button type="button" onClick={resetDraft}>キャンセル</button> : null}
      </div>
    </form>
    <div className="app-usage-limit-settings__registered-apps">
    {settings?.desktopApps.map((entry) => <div key={entry.appId}>
        <div className="app-usage-limit-settings__app-card">
          <h4>{entry.processName}</h4>
          <p className="app-usage-limit-settings__summary">日次 {entry.dailyLimitSeconds / 60}分 ／ 連続 {entry.continuousLimitSeconds / 60}分</p>
          <div className="hourly-rate-settings__app-actions">
